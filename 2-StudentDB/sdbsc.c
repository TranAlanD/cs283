...
int get_student(int fd, int id, student_t *s)
{
    ...
    read_file = read(fd, &temp_student, sizeof(student_t));

    if (read_file == -1) {
        return ERR_DB_FILE;
    }

    if (read_file == 0 || temp_student.id != id) {
        return SRCH_NOT_FOUND; 
    }

	s->id = temp_student.id;
	s->gpa = temp_student.gpa;

	strncpy(s->fname, temp_student.fname, sizeof(s->fname));
	s->fname[sizeof(s->fname) - 1] = '\0';

	strncpy(s->lname, temp_student.lname, sizeof(s->lname));
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    if (read_file == -1) {
        ...
        memset(&temp_student, 0, sizeof(student_t));
    }
    ...
    strncpy(temp_student.fname, fname, sizeof(temp_student.fname) - 1);
    temp_student.fname[sizeof(temp_student.fname) - 1] = '\0';
    strncpy(temp_student.lname, lname, sizeof(temp_student.lname) - 1);
    ...
    write_file = write(fd, &temp_student, sizeof(student_t));
    if (write_file != sizeof(student_t)) {
...
int del_student(int fd, int id)
{
    ...
    int result = get_student(fd, id, &temp_student);
    ...
    write_file = write(fd, &EMPTY_STUDENT_RECORD, sizeof(student_t));
    if (write_file == -1) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd)
{
    ...
    while ((write_file = read(fd, &temp_student, sizeof(student_t))) > 0) {
...
int print_db(int fd)
{
    ...
    while ((read_file = read(fd, &temp_student, sizeof(student_t))) > 0) {
        if (memcmp(&temp_student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            if (!header_printed) {
            ...
            float gpa = temp_student.gpa / 100.0;

            printf(STUDENT_PRINT_FMT_STRING, temp_student.id, temp_student.fname,
                   temp_student.lname, gpa);
        }
    }
