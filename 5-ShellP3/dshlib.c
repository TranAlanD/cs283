...
int exec_local_cmd_loop() {
    ...
    while (1) {
        ...
        rc = build_cmd_list(cmd_buff, cmds, &num_cmds);
        if (rc == WARN_NO_CMDS) {
        ...
        if (strcmp(cmds[0].argv[0], EXIT_CMD) == 0) {
            printf("exiting...\n");
            break;
        }
        ...
        for (int i = 0; i < num_cmds; i++) {
            ...
            if (pids[i] == 0) {
                ...
                execvp(cmds[i].argv[0], cmds[i].argv); // Execute the command
                perror("execvp");
                exit(ERR_EXEC_CMD);
            }
        }
 ...
 */
int build_cmd_list(char *cmd_line, cmd_buff_t *cmds, int *num_cmds)
{
    if (cmds == NULL || cmd_line == NULL || strlen(cmd_line) == 0) {
        return WARN_NO_CMDS;
    }
    ...
    *num_cmds = 0;
	...
	if (*num_cmds > 8) {
            ...
            return -2;  // Custom error code for too many pipes
	}
